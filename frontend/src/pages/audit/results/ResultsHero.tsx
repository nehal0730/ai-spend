...
export default function ResultsHero({ report }: Props) {
  ...
  return (
    <section className="relative overflow-hidden rounded-[2.5rem] bg-[linear-gradient(135deg,rgba(255,255,255,0.94),rgba(241,245,249,0.98))] p-5 shadow-[0_24px_80px_rgba(15,23,42,0.08)] md:p-7 lg:p-8 dark:bg-[linear-gradient(135deg,rgba(15,23,42,0.96),rgba(2,6,23,0.98))] dark:shadow-[0_24px_80px_rgba(2,6,23,0.5)]">
      ...
      <div className="relative grid gap-6 lg:grid-cols-[1.28fr_0.92fr] lg:items-start">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <h1 className="max-w-full">
              <span className="block whitespace-nowrap text-2xl font-extrabold tracking-tight text-primary md:text-3xl lg:text-4xl dark:text-white">Your spend audit,</span>
              <span className="mt-1 block text-2xl font-extrabold tracking-tight text-primary md:text-3xl lg:text-4xl dark:text-white">distilled into a decision surface</span>
            </h1>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <Stat title="Annual savings" value={`$${formatMoney(annualSavings)}`} description="Projected yearly savings" className="border-cyan-400/20 bg-[linear-gradient(180deg,rgba(6,182,212,0.08),rgba(255,255,255,0.92))] dark:bg-[linear-gradient(180deg,rgba(6,182,212,0.14),rgba(15,23,42,0.66))]" />
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
          <div className="rounded-[1.5rem] bg-surface p-4 shadow-[0_12px_36px_rgba(15,23,42,0.08)] backdrop-blur-sm dark:shadow-[0_12px_36px_rgba(2,6,23,0.35)]">
            <div className="flex items-center justify-between gap-3 text-muted">
              <div className="flex items-center gap-2">
                <CalendarDays className="h-4 w-4 text-cyan-600 dark:text-blue-300" />
                <p className="text-sm font-semibold tracking-[0.12em] text-primary dark:text-slate-200">Current spend</p>
              </div>
            ...
            <p className="mt-2 text-2xl font-black text-primary dark:text-white">${formatMoney(report.summary.currentSpend)}</p>
            <p className="mt-1 text-xs leading-5 text-soft">Leadership snapshot for the current billing period.</p>
          </div>
          ...
          {/* <div className="rounded-[1.5rem] border border-white/10 bg-white/5 p-4 shadow-[0_12px_36px_rgba(2,6,23,0.16)] backdrop-blur-sm sm:col-span-2 lg:col-span-1">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <div className="rounded-[1.2rem] bg-white p-4 shadow-[0_12px_30px_rgba(15,23,42,0.06)] dark:bg-slate-950/55 dark:shadow-[0_12px_30px_rgba(2,6,23,0.35)]">
                <p className="text-[11px] font-semibold tracking-[0.14em] text-soft">Rules applied</p>
                <p className="mt-2 text-2xl font-black text-primary dark:text-white">{report.auditMetadata.rulesApplied}</p>
                <p className="mt-1 text-xs text-soft">of {report.auditMetadata.rulesEvaluated} evaluated</p>
              </div>
              <div className="rounded-[1.2rem] bg-white p-4 shadow-[0_12px_30px_rgba(15,23,42,0.06)] dark:bg-slate-950/55 dark:shadow-[0_12px_30px_rgba(2,6,23,0.35)]">
                <p className="text-[11px] font-semibold tracking-[0.14em] text-soft">High confidence</p>
                <p className="mt-2 text-2xl font-black text-primary dark:text-white">{report.auditMetadata.highConfidenceRecommendations}</p>
                <p className="mt-1 text-xs text-soft">recommendations</p>
              </div>
            </div>
          </div> */}
          ...
          <div className="rounded-[1.5rem] bg-[linear-gradient(180deg,rgba(6,182,212,0.08),rgba(255,255,255,0.94))] p-4 shadow-[0_12px_36px_rgba(15,23,42,0.08)] backdrop-blur-sm dark:bg-[linear-gradient(180deg,rgba(56,189,248,0.12),rgba(15,23,42,0.68))] dark:shadow-[0_12px_36px_rgba(2,6,23,0.16)] lg:col-span-1">
            <div className="flex items-center gap-2 text-muted">
              ...
              <p className="text-sm font-semibold tracking-[0.12em] text-primary dark:text-slate-200">Top opportunity</p>
            </div>
            <p className="mt-2 text-xl font-black text-primary dark:text-white">{topTool.name}</p>
            <p className="mt-1 text-sm text-muted dark:text-slate-300">${formatMoney(topTool.spend)} monthly spend, or {topTool.percentage.toFixed(1)}% of the stack.</p>
          </div>
        </div>
      </div>
    </section>
